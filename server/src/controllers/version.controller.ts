  ...
  async list(
    ...
    try {
      ...
      if (!brief) {
        res
          .status(404)
          .json({ success: false, message: "Research brief not found." });
        return;
      }
    ...
    try {
      ...
      const version = await prisma.briefVersion.findFirst({
        where: {
          id: versionId,
          briefId,
          brief: { project: { userId: req.userId! } },
        },
      });
      if (!version) {
  ...
  async save(
    ...
    try {
      ...
      const brief = await prisma.researchBrief.findFirst({
        where: { id: briefId, project: { userId: req.userId! } },
        include: {
          claims: { include: { evidence: { select: { evidenceId: true } } } },
        },
      });
      if (!brief) {
        res
      ...
      const newVersion = await prisma.briefVersion.create({
        data: {
          ...
          changeLog:
            changeLog || `Manual version snapshot v${nextVersionNumber}`,
          briefId,
        },
      });
      ...
      res
        ...
        .json({
          ...
          message: `Version v${nextVersionNumber} saved successfully.`,
        });
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      try {
        ...
        res
          ...
          .json({
            success: false,
            message: "Version content is corrupted or unparseable.",
          });
        return;
      }

      const updatedBrief = await prisma.$transaction(async (tx) => {
        const restoredBrief = await tx.researchBrief.update({
          where: { id: briefId },
          data: {
        ...
        if (Array.isArray(snapshot.claims)) {
          await tx.claimEvidence.deleteMany({ where: { claim: { briefId } } });
          await tx.claim.deleteMany({ where: { briefId } });

          for (const claimSnapshot of snapshot.claims) {
            const claimContent =
              typeof claimSnapshot?.content === "string"
                ? claimSnapshot.content
                : "";
            const claimStatus =
              typeof claimSnapshot?.status === "string"
                ? claimSnapshot.status
                : "PENDING";
            const evidenceIds = Array.isArray(claimSnapshot?.evidenceIds)
              ? claimSnapshot.evidenceIds.filter(
      ...
      res.json({
        ...
        message: `Restored brief to version v${version.versionNumber}.`,
      });
    } catch (error) {
      next(error);
    }
